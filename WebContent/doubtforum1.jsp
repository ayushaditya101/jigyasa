          ...
          <div class="span8">
            ...
            <aside class="right-sidebar">
              ...
              <div class="widget">
               ...
               <form name= "doubt_forum" action="doubtbackend.jsp" method="post" role="form" class="contactForm">
              <div class="row">
                ...
                <div class="span8 form-group">
                  <textarea name="doubt" rows="5" data-rule="required" data-msg="Please write something for us"
...
try{
	...
	rs=st.executeQuery("SELECT MAX(`doubt_id`) FROM `doubts`");
	rs.next();
	id3=rs.getInt(1);   
	
	rs=st.executeQuery("SELECT MIN(`doubt_id`) FROM `doubts`");
   ...
   if(did<id2){
     ...
     <script>
  function myFunction() {
	  if (confirm("You are on the last page!!! Want to go to first page of doubtforum??")) {
	   function redirect(){window.location = "doubtforum.jsp";}
	   redirect();
	  } else {
		  function redirect2(){
			  var s=<%= did+1 %>;
			  window.location = "doubtforum1.jsp?doubt_id="+s;
			  };
		 redirect2();
  ...
  myFunction();
	...
	rs=stmt.executeQuery("SELECT * FROM `doubts` ORDER BY `doubt_id` DESC limit "+(id3-did)+", 5");
	while(rs.next()){
        ...
        <div class="span8" style="text-align:justify;">
            <blockquote><%= doubt %>
              <cite><%= name %></cite>&nbsp &nbsp &nbsp &nbsp &nbsp <a href="reply.jsp?doubt_id=<%= doubt_id %>">Reply</a>
            </blockquote>
          </div>
          <br>
<%
...
%>
           ...
           <a href="doubtforum1.jsp?doubt_id=<%= doubt_id+5 %>" id="b" class="previous">&laquo; Previous</a>
<a href="doubtforum1.jsp?doubt_id=<%= doubt_id-1 %>"  id="b" class="next">Next &raquo;</a>
           </div>
