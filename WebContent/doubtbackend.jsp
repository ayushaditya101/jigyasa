...
String doubt2=request.getParameter("doubt");
...
try{
	...
	/*execute query*/
j=stmt2.executeUpdate("INSERT INTO `doubts`(`name`, `doubt`) VALUES ('"+name2+"','"+doubt2+"')");
	if(j>0){
		    
		
         %>
		<script>
		alert("Thanks for submitting your doubt!!!");
		</script>
...
try{
	...
	rs=stmt.executeQuery("SELECT * FROM `doubts` ORDER BY `doubt_id` DESC limit 5");
	while(rs.next()){
		 name=rs.getString("name");
		 doubt=rs.getString("doubt");
		 id=rs.getString("doubt_id");
...
%>
           ...
           <a id="b" href="#" class="previous">&laquo; Previous</a>
